...
L’approccio proposto identifica automaticamente i microservizi analizzando le API REST esposte da un sistema monolitico. Il metodo si articola in due fasi principali: **calcolo della similarità tra API** e **clustering del grafo di similarità**. Nella prima fase, ogni API viene rappresentata tramite un insieme di *candidate topics* estratti dagli URI e da una descrizione testuale; la similarità semantica è calcolata con una variante del TF-IDF che tiene conto della posizione dei termini. In parallelo viene stimata anche la similarità delle **response messages**, distinguendo tra risposte regolari e risposte basate su riferimenti a definizioni comuni, e considerando l’influenza del metodo HTTP. Le due misure vengono poi combinate tramite un peso α per ottenere la similarità complessiva. Nella seconda fase si costruisce un **grafo di similarità** in cui i nodi rappresentano API e i pesi degli archi indicano la loro affinità; il grafo viene quindi scomposto usando tecniche di **Normalized Cut** e successivamente clusterizzato con **K-means**, scegliendo il numero ottimale di cluster tramite l’indice di Calinski-Harabasz. Ogni cluster risultante rappresenta un microservizio candidato.

L’articolo conclude che l’approccio proposto per l’identificazione automatica dei microservizi, basato sull’analisi della similarità delle API REST, si dimostra efficace grazie alla combinazione della somiglianza semantica tra le API e delle relazioni con le risorse coinvolte. Gli esperimenti su progetti open-source mostrano risultati soddisfacenti e una buona capacità di proporre partizionamenti coerenti. Tuttavia, gli autori riconoscono alcune limitazioni, in particolare la dipendenza da tecniche basate sulla frequenza delle parole, che possono generare interpretazioni errate quando termini diversi indicano lo stesso concetto. Futuri sviluppi includeranno metodi basati sulla semantica dei topic e ulteriori valutazioni in contesti industriali.
...
L’articolo propone **MSExtractor**, un approccio automatico per l’estrazione di microservizi da sistemi legacy, formulato come un problema di **ottimizzazione multi-obiettivo**. Il metodo utilizza l’algoritmo evolutivo IBEA per esplorare lo spazio delle possibili decomposizioni di un’applicazione monolitica, rappresentata come un insieme di classi da suddividere in microservizi coerenti. La tecnica combina **analisi strutturale** (dipendenze tra classi basate su chiamate ai metodi) e **analisi semantica** (similarità calcolata tramite TF-IDF sugli identificatori e commenti del codice), integrandole in una metrica unica di similarità pesata. L’algoritmo ottimizza tre obiettivi: **massimizzare la coesione** interna ai microservizi, **minimizzare l’accoppiamento** tra microservizi e **controllare la granularità**, bilanciando dimensioni e numero dei servizi estratti. MSExtractor distingue inoltre tra **classi interne** e **classi di interfaccia** per calcolare con maggior precisione coesione e accoppiamento. La ricerca procede tramite generazione di soluzioni candidate, crossover e mutazione, producendo partizionamenti del codice valutati secondo le metriche definite. Il metodo è stato validato su sette applicazioni (legacy e microservices-based), mostrando prestazioni superiori rispetto ad approcci esistenti e una buona capacità di recuperare architetture microservizi reali.
